...
1. `tryLock()` 获取锁，获取不到使用 **`scanAndLockForPut`** 方法继续获取。
   ...
   **`scanAndLockForPut`** 方法做的操作就是不断的自旋 `tryLock()` 获取锁。当自旋次数大于指定次数时，使用 `lock()` 阻塞获取锁。

   在自旋时会顺便搜索 hash 位置的链表里有无key相同的 `HashEntry`，确定没有时会创建一个`HashEntry`节点返回。

   另外如果自旋时发现链表被修改，会重新搜索一次链表。

2. 计算 put 的数据要放入的 index 位置，然后获取这个位置上的 `HashEntry` 。

3. 遍历链表，判断 `HashEntry` 的 key 和 hash 值是否和要 put 的 key 和 hash 值一致。

   如果一致则表明找到了key对应的`HashEntry`，替换value的值。

   如果遍历链表没有找到对应key的`HashEntry`，则需要新增一个`HashEntry`节点：

   1. 如果当前容量 + 1 大于扩容阀值，小于最大容量，**则进行扩容**。
   2. 否则直接链表头插法插入。
...
这里第一个 for 是为了寻找链表中最后一段所属位置全部相同的子链表，然后把这一段链表赋值到新位置。第二个 for 循环是为了把剩余的元素通过头插法插入到指定位置链表。
...
<img src="https://oss.javaguide.cn/github/javaguide/java/collection/java8_concurrenthashmap.png" alt="Java8 ConcurrentHashMap 存储结构（图片来自 javadoop）" style="zoom:75%;" />
